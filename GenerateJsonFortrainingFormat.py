...
# data = []
# for i in range(2000):
#     lat = round(random.uniform(51.3, 55.4), 3)  # 随机生成爱尔兰范围内的纬度
#     lon = round(random.uniform(-10.5, -5.3), 3)  # 随机生成经度
#     prompt = f"What is the soil pH at latitude {lat} longitude {lon}?"
#     function_call = {
#         "name": "get_soil_ph",
#         "arguments": {
#             "latitude": lat,
...
#         }
#     }
#     data.append({"input": prompt, "output": function_call})
#
# # 保存为 JSON 格式
# with open("llama_json_finetune.json", "w") as f:
#     json.dump(data, f, indent=4)
#
# print("✅ 训练数据已生成: llama_json_finetune.json")
import torch
import json
import random
...
# ✅ 1️⃣ 加载数据集
with open("llama_json_finetune.json", "r") as f:
    dataset_json = json.load(f)

dataset = Dataset.from_dict({
    "input": [d["input"] for d in dataset_json],
    "output": [json.dumps(d["output"]) for d in dataset_json]
})
print(dataset)
...
def preprocess_function(examples):
    """将 Dataset 转换为 LLaMA 训练格式"""
    inputs = [
        f"Question: {q}\nAnswer:"  # 构造 Prompt 格式
        for q in examples["input"]
    ]
    targets = [
        json.dumps(a)  # 确保输出是 JSON 格式的字符串
...
# 7. 配置训练参数
###############################################################################
save_model_pach="./function_call3"
training_args = TrainingArguments(
    output_dir=save_model_pach,
    overwrite_output_dir=True,
    num_train_epochs=20,               # 先试5个epoch，可根据需要自行调整
