...
upload_url=f'https://farmmaps.eu/api/v1/items/{file_code}/tasks'
headers = {
    "Authorization": f"Bearer {token}",
    # "accept": "application/json",
    # "accept": "*/*"
...
file_code = taskJson[0]["itemCode"]
file_type= taskJson[0]["taskType"]
workflowCode=taskJson[0]["workflowCode"]
print(file_code, file_type, workflowCode)
# response.raise_for_status()
upload_url = f"https://farmmaps.eu/api/v1/Tasks?workflowCode={workflowCode}&itemCode={file_code}&taskType={file_type}&skip=0&take=25"
print(upload_url)
response = requests.get(upload_url, headers=headers)
# response = requests.post(upload_url, headers=headers)
